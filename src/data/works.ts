 ...
 * Migrada desde legacy/js/works.js.
 *
 * Cuando integremos Decap CMS (Fase 4), esta data va a vivir en
 * src/content/trabajos/*.md y se va a generar tipada con content collections.
 * Por ahora la dejamos hardcoded acá para no bloquear la Fase 2.
 */

export type WorkCategory = 'Shows & Fiestas' | 'Marcas' | 'Eventos';
...
export interface Work {
  ...
  year: number;
  location: string;
  /** Path relativo (ej: "/videos/foo.mp4") o URL absoluta */
  video: string;
  cover?: string;
  tagline: string;
  description: string[];
...
export function getWorksByCategory(category: WorkCategory | 'all'): Work[] {
